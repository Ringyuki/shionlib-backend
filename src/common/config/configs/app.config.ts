...
export default (): AppConfig => ({
  ...
  throttle: {
    ...
    download: {
      ...
      limit: parseInt(process.env.THROTTLE_DOWNLOAD_LIMIT || '60'),
      blockDuration: parseInt(process.env.THROTTLE_DOWNLOAD_BLOCK_DURATION_MS || '360000 * 12'),
    },
  },
  ...
  file_upload: {
    ...
    upload_large_file_max_size: parseInt(
      process.env.UPLOAD_LARGE_FILE_MAX_SIZE || '1024 * 1024 * 1024 * 10',
    ), // 10GB
    upload_large_file_max_chunks: parseInt(process.env.UPLOAD_LARGE_FILE_MAX_CHUNKS || '10000'),
    upload_root_dir: process.env.FILE_UPLOAD_ROOT_DIR || '/tmp/shionlib-upload/',
    chunk_size: parseInt(process.env.FILE_UPLOAD_CHUNK_SIZE || '1024 * 1024 * 50'), // 50MB
    upload_session_expires_in: parseInt(
      process.env.FILE_UPLOAD_SESSION_EXPIRES_IN || '1000 * 60 * 60 * 24',
    ), // 1 day
    upload_temp_file_suffix: process.env.FILE_UPLOAD_TEMP_FILE_SUFFIX || '.sltf', // shionlib temp file
    upload_quota: {
      base_size_bytes: parseInt(
        process.env.UPLOAD_QUOTA_BASE_SIZE_BYTES || '5 * 1024 * 1024 * 1024',
      ), // 5GB
      cap_size_bytes: parseInt(
        process.env.UPLOAD_QUOTA_CAP_SIZE_BYTES || '20 * 1024 * 1024 * 1024',
      ), // 20GB
      dynamic_step_bytes: parseInt(
        process.env.UPLOAD_QUOTA_DYNAMIC_STEP_BYTES || '2 * 1024 * 1024 * 1024',
      ), // 2GB
      dynamic_threshold_bytes: parseInt(
        process.env.UPLOAD_QUOTA_DYNAMIC_THRESHOLD_BYTES || '1 * 1024 * 1024 * 1024',
      ), // 1GB
      grant_after_days: parseInt(process.env.UPLOAD_QUOTA_GRANT_AFTER_DAYS || '7'),
      longest_inactive_days: parseInt(process.env.UPLOAD_QUOTA_LONGEST_INACTIVE_DAYS || '30'),
    },
  },
  ...
  file_download: {
    ...
    download_expires_in: parseInt(process.env.FILE_DOWNLOAD_EXPIRES_IN || '3600'), // 1 hour
  },
  ...
  search: {
    engine: (process.env.SEARCH_ENGINE as 'pg' | 'meilisearch' | 'opensearch') || 'pg',
    meilisearch: {
    ...
    opensearch: {
      ...
      auth: process.env.OPENSEARCH_AUTH || 'admin:admin', // username:password
      caPath: process.env.OPENSEARCH_CA_PATH || '',
      indexName: process.env.OPENSEARCH_INDEX_NAME || 'shionlib_games',
    },
  },

  game: {
    hot_score: {
      half_life_release_days: parseInt(process.env.GAME_HOT_SCORE_HALF_LIFE_RELEASE_DAYS || '30'),
      half_life_created_days: parseInt(process.env.GAME_HOT_SCORE_HALF_LIFE_CREATED_DAYS || '15'),
      weight_views: parseFloat(process.env.GAME_HOT_SCORE_WEIGHT_VIEWS || '0.6'),
      weight_downloads: parseFloat(process.env.GAME_HOT_SCORE_WEIGHT_DOWNLOADS || '1.0'),
      weight_release: parseFloat(process.env.GAME_HOT_SCORE_WEIGHT_RELEASE || '0.8'),
      weight_created: parseFloat(process.env.GAME_HOT_SCORE_WEIGHT_CREATED || '0.4'),
    },
  },
})
